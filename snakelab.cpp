...
//原作者网址： https://github.com/tfh93121/C-Snake/blob/master/snake.c
//缺点是使用 cls 命令清屏，导致屏幕显烁得很厉害。
//添加了 gotoxy 函数，实现快速地重复绘制蛇体，解决屏幕闪烁问题。
...
void getKey() {
	if (_kbhit()) {	//如果用户按了键，则获取键并作判断。否则不需处理 
		key = _getch();	
		switch (key) {
			case 'H'://用户按 上 键
				if (direct != 2)
